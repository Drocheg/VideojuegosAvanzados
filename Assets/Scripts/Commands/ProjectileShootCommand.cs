...
public class ProjectileShootCommand {
	...
	public int _id, _maxId, _shooterId;

	public ProjectileShootCommand(int id, int maxId, int shooterId, float x, float y, float z, float dirX, float dirY, float dirZ, float minX, float maxX, float minY, float maxY, float minZ, float maxZ, float positionPrecision) {
		_x = x;
		_y = y;
		_z = z; 
		_dirX = dirX;
	...
	public void Serialize(BitWriter writer) {
		...
		writer.WriteInt((uint) _shooterId, 0, (uint) _maxId);
	...
	public static ProjectileShootCommand Deserialize(BitReader reader, int maxId, float minX, float maxX, float minY, float maxY, float minZ, float maxZ, float positionPrecision) {
		...
		var shooterId = reader.ReadInt(0, maxId);
		...
		return new ProjectileShootCommand(id, maxId, shooterId, x, y, z, dirX, dirY, dirZ, minX, maxX, minY, maxY, minZ, maxZ, positionPrecision);
	}
}
